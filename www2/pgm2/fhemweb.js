...
function addcsrf(arg) {
  ...
  if (csrf && arg.indexOf('fwcsrf') < 0) arg += '&fwcsrf=' + csrf;
  return arg;
}

function FW_cmd(arg) /* see also FW_devState */
{
  arg = addcsrf(arg);
...
function FW_errmsg(txt, timeout) {
  ...
  if (timeout) setTimeout("FW_errmsg('')", timeout);
}

function FW_doUpdate() {
  ...
  if (FW_pollConn.readyState != 3) return;
  ...
  if (FW_pollConn.responseText.length > 300 * 1024) FW_longpoll();
}
...
function FW_longpoll() {
  ...
  if (filter == "") {
    ...
    for (var i = 0; i < embArr.length; i++) {
      ...
      if (svg && svg.firstChild && svg.firstChild.nextSibling && svg.firstChild.nextSibling.getAttribute("flog")) filter = ".*";
    }
  }
  ...
  if (iP != null) filter = filter + ";iconPath=" + iP;

  var query = document.location.pathname + "?XHR=1" + "&inform=type=status;filter=" + filter + "&timestamp=" + new Date().getTime();
  query = addcsrf(query);
...
function FW_selChange(sel, list, elName) {
  var cmd, value;
  var l = list.split(" ");
  for (var i = 0; i < l.length; i++) {
    cmd = l[i];
    var off = cmd.indexOf(":");
    if (off >= 0) cmd = cmd.substring(0, off);
    if (cmd == sel) {
      if (off >= 0) value = l[i].substring(off + 1);
      break;
    }
  }
  ...
  if ((typeof o.qFn == "string")) {
    if (elName.indexOf("val.attr") == 0) FW_queryValue('{AttrVal("' + devName + '","' + sel + '","")}', o.qFn, o.qArg);
